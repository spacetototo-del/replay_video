...
| **지연 재생** | 화면이 계속 "지금으로부터 N초 전"을 재생. 물에서 나와 화면을 보면 방금 뛴 다이빙이 나온다. 지연 10~60초 설정 |
...
| A–B 반복 · 재생 줌 | 시작·끝을 찍으면 그 구간 무한 반복. 재생 화면에서도 두 손가락으로 확대(입수 각도 확인용) |
| 핀치 줌 + 잠금 | 두 손가락으로 카메라 줌(녹화분에도 반영). 세션 중 실수 방지를 위한 줌 잠금(해제는 2초 롱프레스) |
| 터치 기반 화면 디밍 | 일정 시간(기본 30초, 설정 가능) 무터치 → 화면 디밍(배터리 절약). 아무 터치나 하면 즉시 풀밝기 + 타이머 리셋. 첫 실행 후 2분은 유예 |
...
./gradlew :app:testDebugUnitTest                  # 버퍼/타임라인 로직 유닛테스트 (기기 불필요)
./gradlew :app:assembleDebug :wear:assembleDebug
```

버전은 루트 `build.gradle.kts` 의 `appVersionLabel` **한 줄**만 고치면 된다
(모듈 versionCode/versionName, 런처 이름, 앱 내 표시, APK 파일명이 전부 여기서 파생된다).
...
- **Delayed replay lag**: 10 / 15 / 25(기본) / 40 / 60초. 버퍼 길이보다 짧아야 한다.
- **Idle screen**: `Dim when idle`(기본) / `Always bright` / `Screen off when idle`.
  녹화는 어느 모드에서도 멈추지 않으며, 화면만 바뀐다.
  리와인드·지연 재생 화면을 보는 동안에는 디밍 타이머가 멈춘다.

## 알려진 제한 / TODO

- 세그먼트 경계에서 수백 ms 프레임 누락 가능(트레이드오프). 재생·저장 시 이 공백은 건너뛴다.
- 앱 재시작 시 이전 세션 버퍼는 버리고 새로 시작한다(시간 공백 때문).
- 워치 라이브 프리뷰는 미구현(보류).
- 발열 시 경고만 표시하고 화질을 자동으로 낮추지는 않는다 — 재바인드가 버퍼를 날리기 때문.
- 외부 디스플레이(HDMI/무선) 출력, 각도선 드로잉, split 비교는 미구현.
- 아이콘/스플래시 정리, 배터리 실측 남음.
